...
function process(document, target, attrs) {
  ...
  const format = attrs.format || document.getAttribute('javadoc-format', 'xref:attachment$api/java')
  const linkLocation = link(location, target)
  const listDescription = applyFormat(target, format, attrs['$positional'])
  const text = `${linkLocation}['${listDescription}',role=apiref]`
  const role = 'apiref-hint:' + applyFormat(target, 'full').replaceAll(' ', '+')
  return {text, role}
}
...
function link(location, target) {
  ...
  link += target.classReference.replaceAll('.', '/') + '.html'
  if(target.anchor) link += '#' + target.anchor
  return link
}

function applyFormat(target, format, positionalAttrs) {
  if(positionalAttrs?.[0]) return positionalAttrs?.[0]
  switch(format) {
    case 'full':
      return target.classReference + anchorText(target.anchor, format)
    case 'annotation':
      return '@' + shortClassName(target.classReference) + anchorText(target.anchor, format)
    default:
      return shortClassName(target.classReference) + anchorText(target.anchor, format)
  }
}

function anchorText(anchor, format) {
  if (!anchor) return ''
  if (format === 'annotation') anchor = anchor.replaceAll('()', '')
...
function methodText(name, params, format) {
  ...
  return name + '(' + params.split(',').map((name) => (format !== 'full' ? shortClassName(name) : name)).join(', ') + (!varargs ? ')' : '...)')
}

function shortClassName(classReference) {
  return classReference.split('.').slice(-1)
}
