...
function process(document, target, attrs) {
  ...
  const role = 'apiref-hint:' + applyFormat(target, 'full', undefined, format === 'annotation').replaceAll(' ', '+')
  return {text, role}
}
...
function applyFormat(target, format, positionalAttrs, annotationAnchor) {
  ...
  switch(format) {
    case 'full':
      return target.classReference + anchorText(target.anchor, format, annotationAnchor)
    case 'annotation':
