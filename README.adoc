...
In addition, for formats other than `full` a special role of `apiref-hint` will be added to the containing `<code>` element.
This role is not indended to be used in CSS, but is available in case the UI wishes to offer a pop-up hint with the fully-qualified class name.
The format is `apiref-hint:<hint>` where `<hint>` is the hint with spaces replaced by `+`.

NOTE: Ideally a `data` attribute would be used to provide a hint, but this is currently difficult to do with the `html5` asciidoctor backend.
